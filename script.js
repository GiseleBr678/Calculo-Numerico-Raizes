/**
 * Gisele Silva Gomes. 16/07/2024, João Pessoa, Paraíba, Brasil.
 */
...
function evaluateFunction(expression, x) {
  // expression str and x is a number
  const scope = {
    x: x,
    e: Math.exp(1),
    pi: Math.PI,
...
function false_pos(a, b, f, epsilon) {
  ...
  while (true) {
    ...
    const fb = f(b);
    const x =
      (a * Math.abs(fb) + b * Math.abs(fa)) / (Math.abs(fb) + Math.abs(fa));
    const fx = f(x);
...
function secant(a, b, f, epsilon) {
  ...
  while (true) {
    if (iterations > 100) {
      alert("Não foi possível encontrar a raíz.");
      return;
    }
    const fa = f(a);
    const fb = f(b);
...
function solveMethod() {
  ...
  switch (method) {
    ...
    case "newton":
      result = newton(
        a,
        math.parse(funcInput).toString(),
        (x) => evaluateFunction(funcInput, x),
        "x",
        epsilon
      );
      break;
    case "secant":
      ...
      break;
    default:
      alert("Seleção de método inválida.");
      return;
  }
  if (result) {
    document.getElementById(
      "solution"
    ).textContent = `Solução: ${result.raiz.toFixed(6)}`;
    document.getElementById(
      "iterations"
    ).textContent = `Iterações: ${result.iterations}`;
  }
}
